...
class MainActivity : AppCompatActivity() {
    ...
    fun equalClicked(view: View) {
        if (firstNumberText.isEmpty() || secondNumberText.isEmpty() || operatorText.isEmpty()) {
            Toast.makeText(this, "올바르지 않은 수식입니다", Toast.LENGTH_LONG).show()
            return
        }
    ...
    fun operatorClicked(view: View) {
        ...
        if (secondNumberText.isEmpty()) {
            Toast.makeText(this, "1개의 연산자의 결과에 대해서만 연산이 가능합니다.", Toast.LENGTH_LONG).show()
            return
        }
    ...
    private fun updateEquationTextView() {
        val firstFromattedNumber = if (firstNumberText.isNotEmpty()) decimalFormat.format(firstNumberText.toString().toBigDecimal()) else ""
        val secondFormatNumber = if (secondNumberText.isNotEmpty()) decimalFormat.format(secondNumberText.toString().toBigDecimal()) else ""
        binding.equationTextView.text = "$firstFromattedNumber $operatorText $secondFormatNumber"
    }
}